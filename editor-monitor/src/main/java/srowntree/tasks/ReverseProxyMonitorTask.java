...
import srowntree.clients.ReverseProxyClient;
...
@ApplicationScoped
public class ReverseProxyMonitorTask implements Runnable {
    ...
    private static final String[] REVERSE_PROXIES = {"reverse-proxy","reverse-proxy1"};
    ...
    private ReverseProxyClient createReverseProxyClient(String pProxyName) {
        return RestClientBuilder.newBuilder()
                .baseUri(URI.create(PROTOCOL + pProxyName + DOMAIN))
                .property("microprofile.rest.client.disable.default.mapper", true)
                .build(ReverseProxyClient.class);
    }

    public String doMonitoring() {
        SimpleDateFormat formatter= new SimpleDateFormat("yyyy-MM-dd 'at' HH:mm:ss z");
        Date date = new Date(System.currentTimeMillis());
        String outputPrefix = formatter.format(date) + "\nReverse-Proxy Monitoring & Testing:\n";
        ...
        for (String serviceType : SERVICE_TYPES) {
            ...
            for (String service : services) {
                for (String proxy : REVERSE_PROXIES) {
                    ReverseProxyClient reverseProxyClient = createReverseProxyClient(proxy);
                    startTime = System.currentTimeMillis();
                    Response serviceResponse = reverseProxyClient.makeEditorRequest(service, TEST_TEXT);
                    responseTime = System.currentTimeMillis() - startTime;
                    if (responseTime > TOO_LONG_MILLIS) {
                        output += "\n response time: " + responseTime + " was longer than " + TOO_LONG_MILLIS
                                + " for service request type: " + serviceType
                                + " to service " + service + " on proxy: " + proxy + "\n";
                    ...
                    if (serviceType.equals(WORD_COUNT)) {
                        if (answer != WORD_COUNT_ANSWER) {
                            output += "\n answer incorrect, expected " + WORD_COUNT_ANSWER + " but got " + answer
                                    + " for service request type: " + serviceType
                                    + " to service " + service + " on proxy: " + proxy + "\n";
                        } else System.out.println("correct answer from " + service + " on proxy " + proxy);
